...
액션 컨트롤러는 MVC 중 C에 해당하는 부분입니다. 라우팅을 통해 해당 요청을 처리할 컨트롤러가 정해지면 그 컨트롤러는 요청을 이해하고 적절한 출력을 생산하는 임무를 담당합니다. 다행히도 액션 컨트롤러는 대부분의 기초 작업이 되어 있으며 깔금한 규약을 통해 최대한 쉽게 사용할 수 있습니다.

For most conventional RESTful applications, the controller will receive the request (this is invisible to you as the developer), fetch or save data from a model and use a view to create HTML output. If your controller needs to do things a little differently, that's not a problem, this is just the most common way for a controller to work.
대부분의 규약에 의거한 레스트풀(RESTful) 어플리케이션에서 컨트롤러는 요청을 받아서(이 과정은 개발자에게 보이지 않음), 모델을 통해 데이터를 인출 하거나 저장을 하고 HTML 출력을 생성하여 뷰로 사용합니다. 앞서 설명한 부분은 단지 대부분의 컨트롤러의 경우를 말할 뿐, 만약 만들려는 컨트롤러가 약간 다른 무언가를 하길 바란다면 별로 문제될 것이 없습니다.

A controller can thus be thought of as a middle man between models and views. It makes the model data available to the view so it can display that data to the user, and it saves or updates data from the user to the model.
컨트롤러는 마치 모델과 뷰의 중개인과 같은 역할을 합니다. 모델의 데이터를 뷰단에서 이용가능하도록 처리하여 사용자게에 데이터를 보여주고, 사용자로 하여금 데이터를 저장 및 수정하여 모델에 반영할 수 있도록 해줍니다.

NOTE: For more details on the routing process, see "Rails Routing from the Outside In":routing.html.
NOTE: 라우팅 처리과정의 상세한 설명이 필요하시면 "외부 요청에 대한 레일즈 라우팅":routing.html 을 참고하세요. 


h3. Methods and Actions
h3. 메소드(Method)와 액션(Action)

A controller is a Ruby class which inherits from +ApplicationController+ and has methods just like any other class. When your application receives a request, the routing will determine which controller and action to run, then Rails creates an instance of that controller and runs the method with the same name as the action.
컨트롤러는 +ApplicationController+ 를 상속받는 루비 클래스이며 여타 다른 클래스와 같이 여러 메소드를 갖습니다. 어플리케이션이 요청을 받았을 때, 라우팅은 어떤 컨트롤러의 어떤 액션이 실행될지 결정한 다음, 레일즈는 컨트롤러의 인스턴스를 생성, 요청된 액션과 동일하게 명명된 메소드를 실행합니다.
...
예를 들어, 사용자가 어플리케이션에 새로운 클라이언트를 추가하기 위해 +/clients/new+ 호출한다면, 레일즈는 +ClientsController+ 의 인스턴스를 생성하고 +new+ 메소드를 실행시킵니다. 짚고 넘어갈 것은 내용이 없는 빈 메소드라 할지라도 레일즈는 액션에 기타 다른 언급이 없다면 기본적으로 +new.html.erb+ 파일을 뷰로서 렌더링 하므로 잘 작동된다는 것입니다. +new+ 메소드는 뷰에서 +@client+ 인스턴스 변수를 사용할 수 있도록 새로운 +Client+ 만들어낼 수 있습니다.
...
이 부분에 대한 보다 자세한 설명은 레이아웃(Layouts)과 렌더링(Rendering):layouts_and_rendering.html 에 있습니다.

+ApplicationController+ inherits from +ActionController::Base+, which defines a number of helpful methods. This guide will cover some of these, but if you're curious to see what's in there, you can see all of them in the API documentation or in the source itself.
+ApplicationController+ 는 많은 유용한 메소드가 정의되있는 +ActionController::Base+ 를 상속 받습니다. 이 가이드에서 그 부분에 대해 언급은 하겠지만 그 안이 궁금하다면, API 문서, 또는 소스 자체를 통해 모든걸 접할 수 있을 것 입니다.
...
퍼블릭(public)으로 정의된 메소드만이 액션으로서 호출될 수 있습니다. 액션으로서의 목적이 없는 메소드, 즉 부가적인 기능을 담당하거나 필터로서 작동되는 메소드를 외부로 부터 숨기위한 모범적인 방법이죠.
...
컨트롤러의 액션 내부에서 사용자로 부터 전송된 데이터나 다른 파라미터에 접근하는 방법을 설명하겠습니다. 웹 어플리케이션에서는 두가지 종루의 파라미터가 존재하죠. 첫번째로 URL의 일부로 전송되는 것으로, 쿼리 스트링(query string) 파라미터라 부릅니다. URL의 "?" 뒷 부분이 바로 쿼리 스트링이죠. 두번째로 대개 포스트(POST) 데이터로 언급되는 파라미터가 있습니다. 이것들은 대개 사용자에 의해 데이터가 채워진 HTML 폼(form)을 통해 오는 정보 입니다. 포스트(POST) 데이터라 일컫는 이유는, HTTP POST 요청을 통해서만 전송되기 때문입니다. 레일즈는 이 두 종류의 파라미터, 즉 쿼리 스트링(query string) 파라미터와 포스트(POST) 파라미터를 구분하지 않고 +params+ 라는 해쉬를 통해 컨트롤러 내에서 모두 사용 가능합니다.
...
+params+ 해쉬는 1차원적인(one-dimensional keys and values) 구조로 제한되지 않습니다. 여러개의 배열과 (내포된) 여러개의 해쉬를 포함할 수 있습니다. 하나의 배열에 속한 여러 값들을 보내고자 할때는 키 뒷부분에 값이 빈 대괄호 쌍, "[]" 를 덧붙입니다.
...
NOTE: 위 예시의 실제 URL은  "/clients?ids%5b%5d=1&ids%5b%5d=2&ids%5b%5d=3" 와 같이 인코딩될 것인데, 왜냐하면 "[" 와 "]" 는 URL로 허용되지 않기때문이죠. 대개의 경우 브라우져가 알아서 처리해 전송하고, 레일즈는 그걸 받아 디코딩 해주므로 이 부분에 대해 그다지 신경쓸 필요는 없지만 

The value of +params[:ids]+ will now be +["1", "2", "3"]+. Note that parameter values are always strings; Rails makes no attempt to guess or cast the type.
+params[:ids]+ 의 값은 1차원 배열 +["1", "2", "3"]+ 로서 접근 가능해질 것입니다. 주의해야 할 점은 파마리터의 값들은 모두 문자열이라는 것인데요, 레일즈 자체적으로 데이터형을 파악하여 적절하게 형변환을 하지는 않기 때문입니다.

To send a hash you include the key name inside the brackets:
해쉬를 보내고자 할때는 대괄호 내에 키를 넣어주면 됩니다.
<html>
...
이 폼이 전송되면, +params[:client]+ 의 값들은 이렇게 바뀔것 입니다; <tt>{"name" => "Acme", "phone" => "12345", "address" => {"postcode" => "12345", "city" => "Carrot City"}}</tt>. 내포된 해쉬인 +params[:client][:address]+ 도 있네요.
...
h4. 라우팅 파라미터(Routing Parameters)

The +params+ hash will always contain the +:controller+ and +:action+ keys, but you should use the methods +controller_name+ and +action_name+ instead to access these values. Any other parameters defined by the routing, such as +:id+ will also be available. As an example, consider a listing of clients where the list can show either active or inactive clients. We can add a route which captures the +:status+ parameter in a "pretty" URL:
+params+ 해쉬는 항상 +:controller+, +:action+ 두개의 키를 포함할 것입니다. +controller_name+ 와 +action_name+ 메소드로도 접근 가능하죠. 라우팅에 의해 정의된 기타 다른 파라미터, 즉 +:id+ 와 같은 것들도 포함될 것입니다. 하나의 예를 들어, 활성화된 상태, 또는 비활성화된 상태의 클라이언트가 포함된 목록이 있다고 생각해 봅시다. 있도록 하는 
...
이 경우에는, 사용자가 URL +/clients/active+ 을 요청 하면, +params[:status]+ 에는 "active" 가 할당될 것 입니다. 또한 +params[:foo]+ 에는 일반적인 쿼리스트링으로 전달되것들 과 같이 "bar" 가 할당될 것입니다. 같은 맥락에서 +params[:action]+ 에는 "index"가 들어있겠죠.
...
+default_url_options+ 을 통해 전역적인 기본 파마메터 값을 설정할 수 있습니다.  방법은 컨트롤러에 해당이름의 메소드를 정의해 주면 됩니다.
...
이 옵션들은 URL을 생성하는 시작점에서 사용되는데, +url_for+ 에 의해 재정의되기 때문에 가능합니다.  +ApplicationController+ 에 재정의 하면 모든 URL  생성작업에 적용될 것이고, 또는 어떤 특정 컨트롤러에 재정의 함으로서 해당 컨트롤러에 대한 URL 생성시에만 적용할 수도 있습니다.
...
어플리케이션은 각각의 사용자에 대한 세션을 가지고 있습니다. 그곳에 요청 사이에도 사라지지 않은 작은 양의 데이터를 저장할 수 있습니다. 세션은 컨트롤러와 뷰단에서만 사용가능 하고 여러가지의 저장 메카니즘 중 하나를 선택하여 사용할 수 있습니다.

* CookieStore - Stores everything on the client.
* CookieStore - 클라이어트에 모두 저장합니다.
* DRbStore - Stores the data on a DRb server.
* DRbStore - DRb 서버에 저장합니다.
* MemCacheStore - Stores the data in a memcache.
* MemCacheStore - memcache 에 저장합니다.
* ActiveRecordStore - Stores the data in a database using Active Record.
* ActiveRecordStore - 엑티브 레코드를 통해 데이터베이스에 저장합니다.

All session stores use a cookie to store a unique ID for each session (you must use a cookie, Rails will not allow you to pass the session ID in the URL as this is less secure).
위에 열거한 세션 저장 방법 모두 각 세션을 구분짓는 유일한 ID 를 지정하기 위해 쿠키를 사용합니다. (레일스는 보안상의 이유로 URL 에 세션 ID 를 넘기는 것을 허용치 않습니다.)
