...
예를 들어, 사용자가 어플리케이션에 새로운 클라이언트를 추가하기 위해 +/clients/new+ 호출한다면, 레일즈는 +ClientsController+의 인스턴스를 생성하고 +new+ 메소드를 실행시킵니다. 짚고 넘어갈 것은 내용이 없는 빈 메소드라 할지라도 레일즈는 액션에 기타 다른 언급이 없다면 기본적으로 +new.html.erb+ 뷰로서 렌더링 하므로 잘 작동된다는 것입니다. +new+ 메소드는 뷰에서 +@client+ 인스턴스 변수를 사용할 수 있도록 새로운 +Client+ 만들어낼 수 있습니다.
